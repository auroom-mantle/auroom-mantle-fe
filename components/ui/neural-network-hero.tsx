...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Map to warm gold color palette (no black)
    float intensity = (buf[0].x + buf[0].y + buf[0].z) / 3.0;
    
    // Warmer, brighter gold palette
    vec3 deepAmber = vec3(0.6, 0.35, 0.1);    // Deep warm amber (no black)
    vec3 richGold = vec3(0.95, 0.65, 0.15);   // Rich gold
    vec3 brightGold = vec3(1.0, 0.85, 0.35);  // Bright yellow-gold
    vec3 paleGold = vec3(1.0, 0.95, 0.7);     // Pale gold highlights
    
    vec3 color;
    if (intensity < 0.33) {
      color = mix(deepAmber, richGold, intensity * 3.0);
    } else if (intensity < 0.66) {
      color = mix(richGold, brightGold, (intensity - 0.33) * 3.0);
    } else {
      color = mix(brightGold, paleGold, (intensity - 0.66) * 3.0);
    }
...
function ShaderBackground() {
    ...
    return (
        <div ref={canvasRef} className="bg-amber-950 absolute inset-0 -z-10 w-full h-full" aria-hidden>
            <Canvas
                camera={camera}
            ...
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-amber-950/40 via-transparent to-amber-950/30" />
        </div>
    );
}
...
export default function NeuralNetworkHero({
    ...
    microDetails = ["Mantle Network", "ERC-4626 Vault", "Audited Contracts"]
}: HeroProps) {
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
            ...
            <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
                <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 backdrop-blur-sm">
                    <span className="text-[10px] font-light uppercase tracking-[0.08em] text-amber-200/70">{badgeLabel}</span>
                    <span className="h-1 w-1 rounded-full bg-amber-400/60" />
                    <span className="text-xs font-light tracking-tight text-amber-100/90">{badgeText}</span>
                </div>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-amber-950/50 to-transparent" />
        </section>
    );
}
